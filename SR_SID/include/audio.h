 ...
 * @brief	加载音频.
 * @return	成功返回音频对象, 失败返回 NULL.
 * @param [in]	wavfile		音频路径.
 * @param [in]	wavfmt		音频格式, NULL 表示自动检测.
 * @param [in]	smprate		采样率, 当 wavfmt != NULL 时才有效.
 * @param [in]	numchls		通道数, 当 wavfmt != NULL 时才有效.
 * @param [in]	headcut		掐头, 秒.
 * @param [in]	tailcut		去尾, 秒.
 * @param [in]	minproc		最短处理时长, 秒.
 * @param [in]	maxproc		最长处理时长, 秒.
 * @note	当 wavfmt != NULL 时, 认为音频文件无头.
 **************************************************************************************************/
